        ...
        {pendingOrders.length > 0 && (
          <div className="alert alert-warning mb-4">
            ...
            <div className="d-flex gap-2 mb-2">
              ...
              {pendingOrders.some(order => !order.is_paid && order.status !== 'cancelled' && order.status !== 'failed' && (!order.paid_amount || order.paid_amount === 0) && (!order.remaining_balance || order.remaining_balance >= 12900)) && (
                <button
                  className="btn btn-success btn-sm"
                  style={{whiteSpace:'nowrap'}}
                  onClick={async () => {
                    const order = pendingOrders.find(o => !o.is_paid && o.status !== 'cancelled' && o.status !== 'failed' && (!o.paid_amount || o.paid_amount === 0) && (!o.remaining_balance || o.remaining_balance >= 12900));
                    if (!order) return;
